...
void writeFile(std::string fileName, ImageMatrixInt8 imageData, int width, int height);
...
int main(int argc, char* argv[]) {
  ...
  int sigma = 1;
  int higherThreshold = 200;
  int lowerThreshold = higherThreshold / 2;

  if (argc < 7) {
    std::cout << "Syntax Error - Incorrect Parameter Usage:" << std::endl;
    std::cout << "program_name input_image.raw output_image.raw filterSize sigma lower_threshold[0-255] higher_threshold[0-255]" << std::endl;
    return 0;
  }
  ...
  sigma = atof(argv[4]);
  lowerThreshold = atoi(argv[5]);
  higherThreshold = atoi(argv[6]);
  ...
  cv::Mat cvEdgeDetectedImageData(cvImageData.size(), cvImageData.type()), maskImageData(cvImageData.size(), cvImageData.type());

  cv::Canny(cvImageData, cvEdgeDetectedImageData, lowerThreshold, higherThreshold, 3, true);

  ImageMatrixInt8 edgeDetectedImageData(imageHeight, ImageVectorInt8(imageWidth));

  for (int i = 0; i < imageHeight; i++) {
    for (int j = 0; j < imageWidth; j++) {
      edgeDetectedImageData[i][j] = cvEdgeDetectedImageData.at<int8>(i, j);
    }
  }
