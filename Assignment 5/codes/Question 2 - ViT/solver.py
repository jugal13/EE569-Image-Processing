...
class Solver(object):
    ...
    def test(self):
        # if train:
        train_acc, cm = self.test_dataset(self.train_loader)
    ...
    def train(self):
        ...
        optimizer = optim.AdamW(self.model.parameters(),
                                lr=self.args.lr, weight_decay=1e-3)
        linear_warmup = optim.lr_scheduler.LinearLR(
            optimizer, start_factor=1/self.args.warmup_epochs, end_factor=1.0, total_iters=self.args.warmup_epochs, last_epoch=-1, verbose=True)
        cos_decay = optim.lr_scheduler.CosineAnnealingLR(
            optimizer=optimizer, T_max=self.args.epochs-self.args.warmup_epochs, eta_min=1e-5, verbose=True)
        ...
        for epoch in range(self.args.epochs):
            ...
            for i, (x, y) in enumerate(self.train_loader):
                ...
                if i % 50 == 0 or i == (iter_per_epoch - 1):
                    print(
                        f'Ep: {epoch+1}/{self.args.epochs}, It: {i+1}/{iter_per_epoch}, loss: {loss:.4f}')
            ...
            torch.save(self.model.state_dict(), os.path.join(
                self.args.model_path, "ViT_model.pt"))
        ...
        with open("./train.json", "w") as fout:
            json.dump(train_acc_list, fout)

        with open("./test.json", "w") as fout:
            json.dump(test_acc_list, fout)
